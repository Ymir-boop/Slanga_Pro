...
public class Game{
    ...
    // hér eru allar breytur sem koma að leikmönnum
    ...
    /**
     * smiður sem býr til nýjan leik
     *
     * @param size tekur inn parametra fyrir stærð borðsins. Size er fjöldi lína eða dálka borðsins.
     * @param playerAmount parameter fyrir fjölda leikmanna
     */
    public Game(int size, int playerAmount){
        maxSizeOfBoard = size * size;
        snakesNLadders = new SnakesNLadders(maxSizeOfBoard);
        ...
        for (int i = 0; i < playerAmount; i++){
            ...
            if (p == null) return;
            players[i] = p;
            createPlayerMessage(i);
        }
        nextPlayer = players[indexOfPlayer];
        setStartMessage();
    }

    private void createPlayerMessage(int i) {
    ...
    public boolean round(){
        ...
        if (nextPlayer.move(landing, maxSizeOfBoard)) {
            winHandler();
            return true;
        }
        setLandingMessage();
